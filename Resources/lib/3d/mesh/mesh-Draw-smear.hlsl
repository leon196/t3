...
cbuffer TimeParams : register(b5)
{
  float GlobalTime;
  float Time;
  float RunTime;
...
psInput vsMain(uint id: SV_VertexID)
{
    ...
    float t = RunTime + hash11(id)*.1;
    float2 anim = float2(frac(t), floor(t));
    float4 rnd = lerp(hash41(anim.y), hash41(anim.y+1.), pow(anim.x, 10.));
    posInObject.xyz = rndrot(posInObject.xyz, (rnd-.5)*2.);
